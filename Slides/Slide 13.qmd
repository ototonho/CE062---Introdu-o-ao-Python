...
tcrit = stats.t.ppf(1 - alpha/2, df=n_all - 1) # 1 grau de liberdade ??
ci = (xbar - tcrit*se, xbar + tcrit*se)
{"xbar": float(round(xbar, 3)), "IC95%": (float(round(ci[0],3)), float(round(ci[1],3)))}
```
...
ks = stats.kstest((x_all - xbar)/np.std(x_all, ddof=1), 'norm')   # é possível rodar/abrir cada um dos testes para ver se aceita ou não
{"Shapiro-Wilk": {"W": float(round(shapiro.statistic,3)), "p": float(round(shapiro.pvalue,3))},
"Anderson-Darling (norm)": float(round(anderson.statistic,3)),
"KS normalizado": {"D": float(round(ks.statistic,3)), "p": float(round(ks.pvalue,3))}}
```
...
{"t 1-amostra": {"stat": float(round(t1.statistic,3)), "p": float(round(t1.pvalue,3))},
"t Welch 2-amostras": {"stat": float(round(t2.statistic,3)), "p": float(round(t2.pvalue,3))},
"t pareado": {"stat": float(round(tp.statistic,3)), "p": float(round(tp.pvalue,3))}}
```
